...
A :py:class:`.TorchQuantumModel` runs exact non-shot based simulations of quantum circuits, using :term:`torchquantum`'s statevector-based simulator.

.. code-block:: python

   from lambeq import TorchQuantumModel

   model = TorchQuantumModel.from_diagrams(circuits)


To use the :py:class:`.TorchQuantumModel`, you need to install ``lambeq`` with the extra packages by running the following command:

.. code-block:: bash

   pip install lambeq[extras]

:py:class:`.TorchQuantumModel` should be used with the :py:class:`.PytorchTrainer`. :py:class:`.TorchQuantumModel` can be used to optimize simulated circuits using exact backpropagation with PyTorch, which may give improved results over using :py:class:`.NumpyModel` with :py:class:`.SPSAOptimizer`. However, this optimization process is not possible on real quantum hardware, so for more realistic results the parameter-shift rule should be preferred.
