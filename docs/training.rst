...
- :py:class:`.Optimizer`: a ``lambeq`` optimizer calculates the gradient of a given :term:`loss function` with respect to the parameters of a model. It contains a :py:meth:`~lambeq.Optimizer.step` method to modify the model parameters according to the optimizer's update rule. Currently, for the quantum case we support the SPSA algorithm by [Spa1998]_, implemented in the :py:class:`.SPSAOptimizer` class, the Rotosolve algorithm [Oea2021]_ with class :py:class:`.RotosolveOptimizer`, and the Nelder-Mead algorithm [NM1965]_ :cite:p:`gao_2012` with class :py:class:`~lambeq.NelderMeadOptimizer`, while for the classical and hybrid cases we support PyTorch optimizers.
